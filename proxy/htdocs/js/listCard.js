...
function sellCard(id) {
  ...
  $.ajax({
    ...
    success: function (result) {
      console.log(
        "Successfully put card " + id + " for sale for " + price + "$."
      );
      document.getElementById("price-" + id).parentElement.parentElement.style =
...
function addCardToList(
  ...
  let inputPrice = !onSale
    ? "<input type='number' name='price' step='0.01' value='0.00' id='price-" +
      id +
      "'>$"
    : " ";
  ...
  content =
    ...
    "</td>\
    <td>\
        <div class='ui vertical animated button' tabindex='0' onclick='sellCard(" +
    id +
    ")'>\
  ...
  let style = onSale
    ? 'style="background-color:#ccc;cursor:pointer;"'
    : 'style="cursor:pointer";';

  $("#cardListId tr:last").after(
    `<tr id="tr-${id}" ${style} onClick="fillCurrentCard('${imgUrlFamily}','${familyName}','${imgUrl.replace(
      /\//g,
      "/"
    )}','${name}','${description}',${hp},${energy},${attack},${defence},${price})">${content}</tr>`
  );
}
